...
void print(const struct pcap_pkthdr* header, string m_ip_message)
{
	...
	/* 打印时间戳至显示器*/
	cout << 1900 + ltime->tm_year << "-" << 1 + ltime->tm_mon << "-" << ltime->tm_mday << " "; //年月日
	cout << timestr << ","; //时分秒

	/*打印源MAC、源IP、目标MAC、目标IP至显示器*/
	cout << m_ip_message << ",";
	...
	/* 打印时间戳至日志文件*/
	out << 1900 + ltime->tm_year << "-" << 1 + ltime->tm_mon << "-" << ltime->tm_mday << " "; //年月日
	out << timestr << ","; //时分秒

	/*打印源MAC、源IP、目标MAC、目标IP至日志文件*/
	out << m_ip_message << ",";
